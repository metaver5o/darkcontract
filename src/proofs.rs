...
impl ProofHasher {
    ...
    fn add_g2(&mut self, point: &bls::G2Projective) {
...
struct SignatureRequestProofBuilder<'a, R: RngInstance> {
...
impl<'a, R: RngInstance> SignatureRequestProofBuilder<'a, R> {
    ...
    fn commit(&self, hasher: &mut ProofHasher) {
        ...
        hasher.add_g1(&commit_attributes);

        // w_k_j G_1
        // w_m_j h + w_k_j Y
        for (witness_attribute, witness_key) in izip(&self.witness_attributes, &self.witness_keys) {
            let commit_a = self.params.g1 * witness_key;
            let commit_b =
                self.commit_hash * witness_attribute + self.gamma.public_key * witness_key;
    ...
    */
    ...
    fn finish(&self, challenge: &bls::Scalar) -> SignatureRequestProof<'a, R> {
